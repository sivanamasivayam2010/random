...
//checks the functionality of Reset filters button
public class ResetFiltersCheck extends HomePage {
	HomePage home = new HomePage();
	HospitalSearch hospitals;

	@BeforeMethod(alwaysRun = true)
	public void openBrowser() {
		log.info("Hospital Search - resetfilters option Check");
		BaseUI.openBrowser();
		BaseUI.logger = BaseUI.report.createTest("ResetFiltersCheck");
	...
	public void resetFiltersFunctionality() {
		hospitals.clickOnlineBooking();
		hospitals.pageRefreshWait();
		hospitals.clickOpen24x7Filter();
		hospitals.pageRefreshWait();
		hospitals.clickResetFilters();
		hospitals.resetFilterVerify();
	}
